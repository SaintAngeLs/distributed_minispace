...
namespace MiniSpace.Services.Students.Application.UnitTests.Commands.Handlers
{
    public class UpdateUserHandlerTest
    {
        ...
        public async Task HandleAsync_WithValidStudentAndPermitions_ShouldBeUpdated()
        {
            ...
            var command = new UpdateUser(studentId, Guid.NewGuid(), "desc", false);
            ...
            // Act
            await _updateUserHandler.HandleAsync(command, cancelationToken);
        ...
        public async Task HandleAsync_UnknownStudent_ShouldThrowStudentNotFoundException()
        {
            ...
            // Act & Assert
            Func<Task> act = async () => await _updateUserHandler.HandleAsync(command, cancelationToken);
