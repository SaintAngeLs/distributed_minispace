...
{
    public class UserCancelledSignUpToEventHandlerTest
    {
        private readonly UserCancelledSignUpToEventHandler _userCancelledSignUpToEventHandler;
        private readonly Mock<IUserRepository> _userRepositoryMock;
        private readonly Mock<IEventMapper> _eventMapperMock;
        private readonly Mock<IMessageBroker> _messageBrokerMock;

        public UserCancelledSignUpToEventHandlerTest()
        {
            _userRepositoryMock = new Mock<IUserRepository>();
            _eventMapperMock = new Mock<IEventMapper>();
            _messageBrokerMock = new Mock<IMessageBroker>();
            _userCancelledSignUpToEventHandler = new UserCancelledSignUpToEventHandler(
        ...
        public async Task HandleAsync_ValidEvent_ShouldUpdateRepository()
        {
            ...
            var @event = new UserCancelledSignUpToEvent(eventId, studentId);
            ...
            // Act & Assert
            Func<Task> act = async () => await _userCancelledSignUpToEventHandler.HandleAsync(@event, cancelationToken);
