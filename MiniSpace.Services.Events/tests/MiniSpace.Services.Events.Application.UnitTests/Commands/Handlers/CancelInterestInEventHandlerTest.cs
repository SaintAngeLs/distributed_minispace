...
{
    public class CancelInterestInEventHandlerTest
    {
        private readonly CancelInterestInEventHandler _cancelInterestInEventHandler;
        private readonly Mock<IEventRepository> _eventRepositoryMock;
        private readonly Mock<IAppContext> _appContextMock;
        private readonly Mock<IMessageBroker> _messageBrokerMock;

        public CancelInterestInEventHandlerTest()
        {
            _eventRepositoryMock = new Mock<IEventRepository>();
            _appContextMock = new Mock<IAppContext>();
            _messageBrokerMock = new Mock<IMessageBroker>();
            _cancelInterestInEventHandler = new CancelInterestInEventHandler(
        ...
        public async Task HandleAsync_WithValidArguments_ShouldUpdateRepository()
        {
            ...
            var command = new CancelInterestInEvent(eventId, studentId);
            ...
            @event.ShowStudentInterest(new Participant(studentId, studentName));
            ...
            // Act
            await _cancelInterestInEventHandler.HandleAsync(command, cancelationToken);

            // Assert
            _eventRepositoryMock.Verify(repo => repo.UpdateAsync(It.IsAny<Event>()), Times.Once);
            _messageBrokerMock.Verify(msg => msg.PublishAsync(It.IsAny<StudentCancelledInterestInEvent>()), Times.Once);
        }

        [Fact]
        public async Task HandleAsync_NotOvner_ShouldThrowUnauthorizedEventAccessException()
        {
            // Arrange
            var eventId = Guid.NewGuid();
            var studentId = Guid.NewGuid();
            ...
            // Act
            Func<Task> act = async () => await _cancelInterestInEventHandler.HandleAsync(command, cancelationToken);
            ...
            _messageBrokerMock.Verify(msg => msg.PublishAsync(It.IsAny<StudentCancelledInterestInEvent>()), Times.Never);
        }

        [Fact]
        public async Task HandleAsync_WithNotExistingEvent_ShouldThrowEventNotFoundException()
        {
            // Arrange
            var eventId = Guid.NewGuid();
            var studentId = Guid.NewGuid();
