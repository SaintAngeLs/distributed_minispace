...
{
    public class RemoveEventParticipantHandlerTest
    {
        private readonly RemoveEventParticipantHandler _removeEventParticipantHandler;
        private readonly Mock<IEventRepository> _eventRepositoryMock;
        private readonly Mock<IAppContext> _appContextMock;
        private readonly Mock<IMessageBroker> _messageBrokerMock;

        public RemoveEventParticipantHandlerTest()
        {
            _eventRepositoryMock = new Mock<IEventRepository>();
            _appContextMock = new Mock<IAppContext>();
            _messageBrokerMock = new Mock<IMessageBroker>();
            _removeEventParticipantHandler = new RemoveEventParticipantHandler(
        ...
        public async Task HandleAsync_WithValidArguments_ShouldUpdateRepository()
        {
            ...
            var command = new RemoveEventParticipant();
            command.EventId = eventId;
            command.ParticipantId = studentId;
            ...
            @event.AddParticipant(new Participant(studentId, "Adam"));
            ...
            // Act
            await _removeEventParticipantHandler.HandleAsync(command, cancelationToken);

            // Assert
            _eventRepositoryMock.Verify(repo => repo.UpdateAsync(It.IsAny<Event>()), Times.Once);
            _messageBrokerMock.Verify(msg => msg.PublishAsync(It.IsAny<EventParticipantRemoved>()), Times.Once);
        }
        [Fact]
        public async Task HandleAsync_WithoutExistingEvent_ShouldThrowEventNotFoundException()
        {
            ...
            // Act
            Func<Task> act = async () => await _removeEventParticipantHandler.HandleAsync(command, cancelationToken);
            ...
            _messageBrokerMock.Verify(msg => msg.PublishAsync(It.IsAny<EventParticipantRemoved>()), Times.Never);
        }
        [Fact]
        ...
        [Fact]
        public async Task HandleAsync_NotBeingOrganizerButAdmin_ShouldThrowNotUnauthorizedEventAccessException()
        {
            // Arrange
            var eventId = Guid.NewGuid();
            var studentId = Guid.NewGuid();
