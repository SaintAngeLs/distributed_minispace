...
{
    public class SignUpToEventHandlerTest
    {
        private readonly SignUpToEventHandler _signUpToEventHandler;
        private readonly Mock<IEventRepository> _eventRepositoryMock;
        private readonly Mock<IStudentRepository> _studentRepositoryMock;
        private readonly Mock<IAppContext> _appContextMock;
        private readonly Mock<IMessageBroker> _messageBrokerMock;

        public SignUpToEventHandlerTest()
        {
            _eventRepositoryMock = new Mock<IEventRepository>();
            _studentRepositoryMock = new Mock<IStudentRepository>();
            _appContextMock = new Mock<IAppContext>();
            _messageBrokerMock = new Mock<IMessageBroker>();
            _signUpToEventHandler = new SignUpToEventHandler(
        ...
        public async Task HandleAsync_WithValidArguments_ShouldUpdateRepository()
        {
            ...
            var command = new SignUpToEvent(eventId, studentId);
            ...
            // Act
            await _signUpToEventHandler.HandleAsync(command, cancelationToken);

            // Assert
            _eventRepositoryMock.Verify(repo => repo.UpdateAsync(It.IsAny<Event>()), Times.Once);
            _messageBrokerMock.Verify(msg => msg.PublishAsync(It.IsAny<StudentSignedUpToEvent>()), Times.Once);
        }
        [Fact]
        public async Task HandleAsync_WithoutExistingEvent_ShouldThrowEventNotFoundException()
        {
            ...
            // Act
            Func<Task> act = async () => await _signUpToEventHandler.HandleAsync(command, cancelationToken);
            ...
            _messageBrokerMock.Verify(msg => msg.PublishAsync(It.IsAny<StudentSignedUpToEvent>()), Times.Never);
        }
        [Fact]
